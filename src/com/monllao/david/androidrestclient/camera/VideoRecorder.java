...
public class VideoRecorder {
    ...
    protected void setPreviewView() {
        ...
        // Getting the camera
        try {
            camera = Camera.open(AndroidRestClientActivity.CAMERA_ID);
        } catch (Exception e) {
    ...
    protected void calculateOptimalScreenSize() {
        ...
        Log.v(AndroidRestClientActivity.APP_NAME, "Screen size: " + screenWidth + ":" + screenHeight);

        height = 0;
        width = 0;
        
        final List<Camera.Size> supportedSizes = camera.getParameters().getSupportedPreviewSizes();
        ...
        while (it.hasNext()) {
            ...
            } else if (screenWidth > size.width && screenHeight > size.height && height == 0 && width == 0) {
                height = size.height;
                width = size.width;
            }
        }

        Log.i(AndroidRestClientActivity.APP_NAME, "Selected size: " + width+ ":" + height);
    }
